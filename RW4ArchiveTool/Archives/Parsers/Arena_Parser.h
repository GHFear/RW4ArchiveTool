...
namespace arena
{
    ...
    std::vector<Archive_Parse_Struct> parse_arena_filepackage(const wchar_t* archiveName, const wchar_t* directory, bool unpack) {
        ...
        file_count = dword_big_to_little_endian(file_count);

        _fseeki64(archive, 48, SEEK_SET);

        uint32_t file_table_offset = 0;
        fread(&file_table_offset, sizeof(uint32_t), 1, archive);
        file_table_offset = dword_big_to_little_endian(file_table_offset);
        ...
        for (size_t i = 0; i < file_count; i++)
        {
            ...
            size = qword_big_to_little_endian(size);

            uint64_t unknown1 = 0;
            fread(&unknown1, sizeof(uint32_t), 1, archive);
            unknown1 = dword_big_to_little_endian(unknown1);

            uint64_t unknown2 = 0;
            fread(&unknown2, sizeof(uint32_t), 1, archive);
            unknown2 = dword_big_to_little_endian(unknown2);

            uint32_t file_type = 0;
            fread(&file_type, sizeof(uint32_t), 1, archive);
            file_type = dword_big_to_little_endian(file_type);

            size_t object_name_index = 0;
            for (size_t j = 0; j < sizeof(ArenaObjectTypeArray) / sizeof(ArenaObjectTypeArray[0]); j++)
            {
                if (dword_big_to_little_endian(ArenaObjectTypeArray[j]) == file_type)
                {
                    object_name_index = j;
                    name_found = true;
                    break;
            ...
            if (offset == 0)
            {
                ...
                local_size = dword_big_to_little_endian(local_size);
